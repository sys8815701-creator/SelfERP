    ...
    <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        <div>
          ...
          <p style={{ fontSize: "13px", color: "var(--text-muted)" }}>실사 수량을 입력하면 장부와 자동 비교 후 조정됩니다. · {items.length}개 품목</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <button onClick={handleApply} disabled={saving || !hasDiff || loading}
            style={{ backgroundColor: hasDiff ? "var(--accent)" : "var(--bg-surface-2)", color: hasDiff ? "var(--accent-text)" : "var(--text-muted)", border: "none", borderRadius: "8px", padding: "9px 18px", fontSize: "13px", fontWeight: 700, cursor: saving || !hasDiff ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
            {saving ? "적용 중..." : "조정 적용"}
          </button>
        </div>
      </div>
      ...
      {result && (
        <div style={{ backgroundColor: "#DCFCE7", border: "1px solid #86EFAC", borderRadius: "12px", padding: "14px 18px", marginBottom: "18px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
            {result.filter(r => r.action === "adjusted").map((r: any) => (
              <span key={r.item_id} style={{ fontSize: "12px", padding: "3px 10px", backgroundColor: "white", borderRadius: "20px", color: "#166534", border: "1px solid #86EFAC" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "1px solid var(--border)", backgroundColor: "var(--bg-surface-2)" }}>
              {["품목명", "유형", "단위", "장부 수량", "실사 수량", "차이", "메모"].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} style={{ padding: "48px", textAlign: "center", color: "var(--text-muted)", fontSize: "13px" }}>등록된 품목이 없습니다.</td></tr>
            ) : items.map((item, i) => {
              ...
              return (
                <tr key={item.id}
                  style={{ borderBottom: i < items.length - 1 ? "1px solid var(--border-subtle)" : "none",
                    backgroundColor: hasDiffRow ? (diff > 0 ? "rgba(220,252,231,0.3)" : "rgba(254,242,242,0.3)") : "transparent" }}>
      ...
      {hasDiff && !loading && (
        <div style={{ marginTop: "12px", padding: "12px 16px", backgroundColor: "var(--bg-surface-2)", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button onClick={handleApply} disabled={saving}
            style={{ backgroundColor: "var(--accent)", color: "var(--accent-text)", border: "none", borderRadius: "8px", padding: "8px 16px", fontSize: "13px", fontWeight: 700, cursor: "pointer" }}>
            {saving ? "적용 중..." : "조정 적용"}
          </button>
        </div>
      )}
