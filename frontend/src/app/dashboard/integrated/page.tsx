...
import { Alert, fetchAlerts, getDismissedAlertIds } from "@/lib/alerts";
...
export default function IntegratedDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get("/api/hr/payroll/", { headers: h, params: { year: now.getFullYear(), month: now.getMonth() + 1 } }).catch(() => ({ data: [] })),
      // 회계
      ...
      // 알림 — 알림 센터(/dashboard/integrated/alerts)와 완전히 동일한 소스 + id 체계를 써서,
      // 한쪽에서 dismiss한 알림이 다른 쪽에서도 항상 같은 항목으로 걸러지도록 한다.
      fetchAlerts(bizId),
    ]).then(([emp, payroll, ar, ap, prod, eff, dist, fetchedAlerts]) => {
      setData({
        emp:       emp.data,
      ...
      setAlerts((fetchedAlerts as Alert[]).filter(a => !dismissed.has(a.id)));
      setLoading(false);
    });
  }, []);
  ...
  const MODULES = [
    {
      ...
      kpis: [
        { label: "재직 직원", value: loading ? "—" : `${(data.emp ?? []).filter((e: any) => e.status === "재직").length}명` },
        { label: "이번 달 급여", value: loading ? "—" : fmtW((data.payroll ?? []).reduce((s: number, p: any) => s + (p.net_pay ?? 0), 0)) },
      ],
    },
    {
  ...
  if (role !== "admin") {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "50vh", gap: "10px", color: "var(--text-muted)" }}>
        ...
        <p style={{ fontSize: "13px" }}>통합 대시보드는 사업장 관리자(admin)만 확인할 수 있습니다.</p>
      </div>
    );
  }
            ...
            return (
              <div key={a.id} onClick={() => router.push(a.href)}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <span style={{ fontSize: "13px", color: "var(--text-primary)", fontWeight: 500 }}>{a.title}</span>
                  {a.desc && <span style={{ fontSize: "12px", color: "var(--text-muted)", marginLeft: "6px" }}>— {a.desc}</span>}
                </div>
