...
def get_role_access(
    ...
    setting = db.query(SystemSetting).filter(SystemSetting.key == "role-access").first()
...
async def save_role_access(
    ...
    if setting:
        ...
        setting = SystemSetting(key="role-access", value=json.dumps(data, ensure_ascii=False))
        db.add(setting)
    db.commit()
